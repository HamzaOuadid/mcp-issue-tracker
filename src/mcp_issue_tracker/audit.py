...
compact JSON object per line) and mirrored into a SQLite ``audit_log``
table matching the spec's data model, so the same history can be tailed
as text or queried with SQL. Same dual-sink design as the sibling
`mcp-starter-template` project's ``audit.py``.
"""
